...
const handleCancelBooking = useCallback((booking: BookingRow) => {
  ...
  const attentionBookings = useMemo(() => {
    return bookings
      .filter(
        (booking) =>
          canLeaveReview(booking) || canReportTeacherNoShow(booking),
      )
      .sort((a, b) => {
    ...
    return bookings
      .filter(
        (b) =>
          ...
          (
            !b.session_start_time ||
            isPast(b.session_end_time ?? b.session_start_time) ||
            isPendingBookingExpired(b) ||
            b.booking_status === "CANCELLED_BY_LEARNER" ||
            b.booking_status === "CANCELLED_BY_TEACHER" ||
            b.booking_status === "LATE_CANCELLED_BY_LEARNER" ||
...
{showReviewButton || showTeacherNoShowButton ? (
            <View style={styles.postSessionCard}>
              ...
              <Text style={styles.postSessionBody}>
                Leave a review if everything went well. If the teacher did not
                attend, report the issue within 24 hours of the session start.
              </Text>
                ...
                {attentionBookings.length > 0 ? (
                  <View style={styles.sectionWrap}>
                    ...
                    <Text style={styles.attentionSectionSubtitle}>
                      Confirm how your recent session went or report an issue
                      within the available review period.
                    </Text>
